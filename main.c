...
u8* window_positions_table = window_positions_table_1;
u8* window_positions_table_next = window_positions_table_2;*/




// TODO Use a single window for smaller hdma table ?
// HDMA Mode 1 : 2 registers (W1Left W1Right)
...
// TODO Indirect HDMA ? 2x 112 lines ==> pointing to a single 224 lines array (with 2 bytes per entry for L and R registers)


/*! \fn  replaceHdmaTable(u8* hdmatableR)
    \brief Replace HDMA channel 2 table.
    \param hdmatableL table with windows effect on the left/right for both windows
*/
    ...
    while (1)
    {
		...
		if (playersMoved) {
			...
			if (camera_diff == 0) {
				...
				// TODO Proper asm routine for hardware division / wait cycles
			...
			*/
			...
			/*
			u8* ptr = window_positions_table/*_next*//* + 5;
			u8 split = split_x >> 8;
			u8 y;
			for (y=0; y<SCREEN_HEIGHT; y++) {
				if ((y%60) == 0) {
			...
			u8 y;
			for (y=0; y<SCREEN_HEIGHT; y++) {
				//if ((y%60) == 0) {	// FIXME Avoid modulo / division by using binary mask?
				if ((y & 0x3f) == 0) {
					ptr++;
				}
				// FIXME simplify this mess...
			...
			// Swap hdma tables
			u8* temp = window_positions_table;
			window_positions_table = window_positions_table_next;
			window_positions_table_next = temp;
			replaceHdmaTable(window_positions_table);
			*/
